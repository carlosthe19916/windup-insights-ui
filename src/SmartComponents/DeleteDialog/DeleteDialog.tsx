...
class DeleteDialogBase extends React.Component<Props, State> {
  ...
  public render() {
    const { type, name, onDelete, onCancel, isOpen, isProcessing, isError } = this.props;
    ...
    return (
      <Modal
        ...
        isOpen={isOpen}
        actions={[
          <Button key="confirm" isDisabled={isProcessing} variant={ButtonVariant.danger} onClick={onDelete}>
            Delete {`${type}`}
          </Button>,
          <Button key="cancel" isDisabled={isProcessing} variant={ButtonVariant.link} onClick={onCancel}>
            Cancel
          </Button>
        ]}
      >
